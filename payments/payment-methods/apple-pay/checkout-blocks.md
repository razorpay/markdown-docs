...
- **Order Creation via Backend**: Your server must be able to create orders using the [Orders API](https://raw.githubusercontent.com/razorpay/markdown-docs/master/api/orders/create.md) and pass the `order_id` to the frontend.
...
Before initiating a payment, create an order on your backend using the [Razorpay Orders API](https://raw.githubusercontent.com/razorpay/markdown-docs/master/api/orders/create.md). Pass the resulting `order_id` to your frontend to initialise the SDK.
...
Refer to the [Orders API documentation](https://raw.githubusercontent.com/razorpay/markdown-docs/master/api/orders/create.md) for detailed request and response parameters.
