...
You can create [customers](https://raw.githubusercontent.com/razorpay/markdown-docs/master/api/customers.md) with basic information such as `email` and `contact` and use them for various Razorpay offerings. The following endpoint creates a customer.
...
You can use the [Orders](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/orders.md) API to create a unique Razorpay `order_id` that is associated with the authorisation transaction. The following endpoint creates an order.
...
: Details related to the authorisation transaction such as max amount and bank account information. Pass a value in the `first_payment_amount` parameter if you want to auto-charge the customer the first payment immediately after authorization.
 ...
 `auth_type` _mandatory_
  ...
  : `integer` Use to set the maximum amount per debit request. Know about [maximum and default values](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/recurring-payments/paper-nach/faqs.md#3-is-there-a-limit-on-the-debit).
...
: Details related to the authorisation such as max amount and bank account information.
  ...
  `max_amount`
  : `integer` The maximum amount in paise a customer can be charged in a transaction. Know about the [maximum and default values](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/recurring-payments/emandate/faqs.md#2-what-is-the-maximum-amount-which-can).
...
Registration Link is an alternate way of creating an authorisation transaction. You can create a registration link using the [API](#121-create-a-registration-link) or [Dashboard](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/recurring-payments/create.md#1-create-a-registration-link).
...
- When you create a registration link, an [invoice](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/invoices.md) is automatically issued to the customer. They can use this invoice to make the authorisation payment.
...
> **Handy Tips**
> 
> You can [use Webhooks to get notifications about successful payments](https://raw.githubusercontent.com/razorpay/markdown-docs/master/api/payments/recurring-payments/webhooks.md#check-authorization-link-status-using-webhooks) against a registration link.
> 
...
: Details of the authorisation payment.
  ...
  `max_amount` _optional_
  : `integer` Use to set the maximum amount, in paise, per debit request. Know about [maximum and default values](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/recurring-payments/paper-nach/faqs.md).
...
You can retrieve the `token_id` using the [Dashboard](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/recurring-payments/create.md#3-search-for-the-token) or the APIs given below.

Know more about [Tokens](https://raw.githubusercontent.com/razorpay/markdown-docs/master/payments/recurring-payments/paper-nach/integrate.md#fetch-nach-mandate-registration-details).
...
: `boolean` Determines whether the payment status should be changed to `captured` automatically or not. Possible values:
        ...
        - `false`: Payments are not captured automatically. You can manually capture payments using the [Manually Capture Payments API](https://raw.githubusercontent.com/razorpay/markdown-docs/master/api/payments.md#capture-a-payment).
